...
fn test_success_etoc_tx_with_different_setters_getters() {
    ...
    // use separate cache folders for v1 versus v2 tests
    let test_cache_folder = format!("{}/{}", CONTRACT_CACHE_FOLDER, "v1_test");
    let runtime = pchain_runtime::Runtime::new()
        .set_smart_contract_cache(Cache::new(Path::new(&test_cache_folder)));
...
/// Verify the data is correctly read/write from/to world state with different getter and setter.
#[test]
fn test_success_etoc_tx_with_different_setters_getters_v2() {
    let contract_code = TestData::get_test_contract_code("basic_contract");
    let origin_address = [1u8; 32];
    ...
    // use separate test folders for v1 versus v2 tests
    let test_cache_folder = format!("{}/{}", CONTRACT_CACHE_FOLDER, "v2_test");
    let runtime = pchain_runtime::Runtime::new()
        .set_smart_contract_cache(Cache::new(Path::new(&test_cache_folder)));
    ...
    assert!(result.error.is_none());
    let rcp = result.receipt.as_ref().expect("Receipt expected");
    assert!(verify_receipt_content_v2(
        rcp,
        1393335,
        1258695,
    ...
    if let Some(CommandReceiptV2::Call(cr)) = rcp.command_receipts.last() {
        assert_eq!(cr.exit_code, ExitCodeV2::Ok);
        assert_eq!(
            CallResult::parse::<u64>(cr.return_value.clone()).unwrap(),
            5678_u64
...
fn test_success_etoc_multiple_methods_v2() {
    let contract_code = TestData::get_test_contract_code("basic_contract");
    let origin_address = [1u8; 32];
    let contract_address = contract_address_v2(&origin_address, 0, 0);
...
/// Verify key of the fields in nested struct is correctly loaded/written
#[test]
fn test_success_etoc_set_all_contract_fields_v2() {
    let contract_code = TestData::get_test_contract_code("all_features");
    let origin_address = [1u8; 32];
    let contract_address = contract_address_v2(&origin_address, 0, 0);
...
/// Simulate test to check data access to Network Account Storage.
#[test]
fn test_success_etoc_network_state_v2() {
    let contract_code = TestData::get_test_contract_code("all_features");
    let origin_address = [1u8; 32];
    let contract_address = contract_address_v2(&origin_address, 0, 0);
    let network_state_storage_key = vec![5u8];
    ...
    if let Some(CommandReceiptV2::Deploy(cr)) = rcp.command_receipts.last() {
        assert_eq!(cr.exit_code, ExitCodeV2::Ok);
    } else {
        panic!("Deploy command receipt expected");
    }
...
/// Verify the transaction status code is FailureRuntimeError and no state has been setted.
#[test]
fn test_failure_etoc_tx_with_invalid_argument_data_type_v2() {
    let contract_code = TestData::get_test_contract_code("basic_contract");
    let origin_address = [1u8; 32];
    let contract_address = contract_address_v2(&origin_address, 0, 0);
...
/// Verify the transaction status code is FailureRuntimeError and no state is setted.
#[test]
fn test_failure_etoc_tx_with_invalid_method_name_v2() {
    let contract_code = TestData::get_test_contract_code("basic_contract");
    let origin_address = [1u8; 32];
    let contract_address = contract_address_v2(&origin_address, 0, 0);
...
/// Simulate test to call crypto functions in smart contract.
#[test]
fn test_success_etoc_crypto_functions_v2() {
    let contract_code = TestData::get_test_contract_code("all_features");
    let origin_address = [1u8; 32];
    let contract_address = contract_address_v2(&origin_address, 0, 0);
