...
pub const fn wasm_memory_read_cost(len: usize) -> u64 {
    ...
    if cost == 0 { return 1 } // = max(cost, 1) to make sure charging for a non-zero cost 
    cost 
}
...
pub const fn set_cost_delete_old_value(key_len: usize, old_val_len: usize, new_val_len: usize) -> u64 {
    let old_val_len = old_val_len as u64; // (a)
    let new_val_len = new_val_len as u64; // (b)

    if (old_val_len > 0 || old_val_len == 0) && new_val_len > 0 {
        // a * C_write * C_refund 
        old_val_len.saturating_mul(MPT_WRITE_PER_BYTE_COST * MPT_WRITE_REFUND_PROPORTION).saturating_div(100)
    } else if old_val_len > 0 && new_val_len == 0 {
        // (k + a) * C_write * C_refund 
        ((key_len as u64).saturating_add(old_val_len)).saturating_mul(MPT_WRITE_PER_BYTE_COST * MPT_WRITE_REFUND_PROPORTION).saturating_div(100) 
