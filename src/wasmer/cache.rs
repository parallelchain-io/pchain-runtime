...
impl Cache {
    ...
    pub(crate) fn load(&self, address: PublicAddress, store: &wasmer::Store) -> Result<(Module, ModuleMetadata), DeserializeError> {
        let key = wasmer_cache::Hash::new(address);
        let file_storage = self.inner.try_read().map_err(|_| 
            DeserializeError::Io(Error::from(ErrorKind::Interrupted))
        )?;

        let module = unsafe { file_storage.load(store, key)? };
        let metadata = file_storage.metadata(key).map_err(|_| {
            DeserializeError::Io(Error::from(ErrorKind::NotFound))
        })?;
    ...
    pub(crate) fn store(&mut self, address: PublicAddress, module: &wasmer::Module, bytes_length: usize) -> Result<(), SerializeError> {
        let key = wasmer_cache::Hash::new(address);
        let mut file_storage = self.inner.try_write().map_err(|_| 
            SerializeError::Io(Error::from(ErrorKind::Interrupted))
        )?;
...
impl FileStorage {
    ...
    fn set_metadata(&mut self, key: wasmer_cache::Hash, metadata: ModuleMetadata) -> Result<(), ()> {
        let path = self.metadata_path.join(key.to_string());
        let mut file = std::fs::OpenOptions::new().write(true).create(true).open(path).map_err(|e| {
            println!("{:?}", e);
        }).unwrap();
...
impl WasmerCache for FileStorage {
    ...
    unsafe fn load(&self, store: &wasmer::Store, key: wasmer_cache::Hash) -> Result<Module, Self::DeserializeError> {
        self.fs_cache.load(store, key)
    }

    fn store(&mut self, key: wasmer_cache::Hash, module: &Module) -> Result<(), Self::SerializeError> {
