...
use crate::{
    ...
    TransitionError, 
    gas, formulas::{TREASURY_CUT_OF_BASE_FEE, TOTAL_BASE_FEE}
};
...
pub(crate) fn finalize_gas_consumption<S>(mut state: ExecutionState<S>) -> Result<ExecutionState<S>, StateChangesResult<S>>
...
pub(crate) fn abort<S>(mut state: ExecutionState<S>, transition_err: TransitionError) -> StateChangesResult<S>
    where S: pchain_world_state::storage::WorldStateStorage + Send + Sync + Clone + 'static
{
...
pub(crate) fn charge<S>(mut state: ExecutionState<S>, transition_result: Option<TransitionError>) -> StateChangesResult<S>
    where S: WorldStateStorage + Send + Sync + Clone + 'static
{
    let signer = state.tx.signer;
    let base_fee = state.bd.this_base_fee;
