    ...
    where S: WorldStateStorage + Send + Sync + Clone
{
    ...
    let new_validator_set = 
    {
        ...
        for pool in &pools_in_vp {
            ...
            let stats = block_performance.stats.get(&pool_operator)
            ...
            if pool_reward > 0 {
                for stake in &vp_stakes {
                    let (stake_reward, commission_fee) = stake_reward(pool_reward, commission_rate, stake.power, total_stakes);
            ...
            for (stake_owner, reward) in stakers_to_reward {
                let mut stake_owner_deposit = NetworkAccount::deposits(&mut state, pool_operator, stake_owner);
            ...
            if pool_reward > 0 {
                let (pool_operator_stake_reward, _) = stake_reward(pool_reward, 0, pool_operator_own_stake, total_stakes);
                let mut operator_deposits = NetworkAccount::deposits(&mut state, pool_operator, pool_operator);
                let pool_operator_total_reward = pool_operator_stake_reward.saturating_add(total_commission_fee);
                match operator_deposits.balance() {
                    Some(balance) => {
                        operator_deposits.set_balance(balance.saturating_add(pool_operator_total_reward));
                    },
                    None => {
        ...
        let new_validator_set: Vec<(PublicAddress, u64)> = next_validator_set.iter().filter_map(|(new_p, new_power)|{
            if !pools_in_vp.iter().any(|old_p| old_p.operator == *new_p && old_p.power == *new_power) {
                Some((*new_p, *new_power))
            } else { None }
        }).collect();
        let remove_validator_set = pools_in_vp.iter().filter_map(|old_p| {
            if !next_validator_set.iter().any(|new_p| new_p.0 == old_p.operator) {
                Some(old_p.operator)
            } else { None }
        }).collect();
...
fn pool_reward(
    ...
    let (numerator, denominator) = issuance_reward(current_epoch, pool_power);
    (
        (numerator * num_of_proposed_blocks as u128)
        / (denominator * baseline as u128)
    ) as u64
}


/// return reward to the stakes and commission_fee
fn stake_reward(pool_reward: u64, commission_rate: u8, stake_power: u64, total_stakes: u64) -> (u64, u64) {
    // no reward if there is no stakes at all
    if total_stakes == 0 { return (0, 0) }
    let reward = 
        (pool_reward as u128 * stake_power as u128) 
        / (total_stakes as u128);
    let commission_fee = 
        (commission_rate as u128 * pool_reward as u128 * stake_power as u128) 
        / (100 * total_stakes as u128);
    ((reward.saturating_sub(commission_fee)) as u64, commission_fee as u64 )
}