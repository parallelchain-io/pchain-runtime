...
use pchain_types::{cryptography::PublicAddress, blockchain::{Transaction, CommandReceipt, Command, Receipt, Log, ExitStatus}};
...
use crate::{
    execution::{state::ExecutionState, execute}, 
    types::{BaseTx, DeferredCommand}, 
    wasmer::cache::Cache, 
    TransitionError, contract::SmartContractContext, BlockchainParams, cost::CostChange, read_write_set::ReadWriteSet
};
...
impl Runtime {
    ...
    pub fn transition<S: WorldStateStorage + Send + Sync + Clone + 'static>(
        ...
        let state = ExecutionState { tx: base_tx, tx_size, commands_len: commands.len(), ctx, bd };
...
impl<S> StateChangesResult<S>
    where S: WorldStateStorage + Send + Sync + Clone + 'static
{
    pub(crate) fn new(state: ExecutionState<S>, transition_error: Option<TransitionError>) -> StateChangesResult<S> {
        Self {
            state,
