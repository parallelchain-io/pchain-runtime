...
use pchain_world_state::{storage::WorldStateStorage, states::{WorldState, AccountStorageState}, keys::AppKey};
use wasmer::Store;

use crate::{cost::CostChange, gas, contract::{self, SmartContractContext, Module}};
    ...
    where S: WorldStateStorage + Send + Sync + Clone
{
    ...
    pub fn contains_app_data_from_account_storage_state(&self, account_storage_state: &AccountStorageState<S>, app_key: AppKey) -> bool {
        ...
        self.writes.get(&cache_key).filter(|v| v.len() != 0).is_some()
        || self.reads.borrow().get(&cache_key).filter(|v| v.is_some()).is_some()
        || self.ws.contains().storage_value_from_account_storage_state(account_storage_state, &app_key)
    }

    /// Get app data given a account storage state. It is gas-free operation.
    pub fn app_data_from_account_storage_state(&self, account_storage_state: &AccountStorageState<S>, app_key: AppKey) -> Option<Vec<u8>> {
        let address = account_storage_state.address();
        let cache_key = CacheKey::App(address, app_key.clone());
        ...
        self.ws.cached_get().storage_value(account_storage_state.address(), &app_key)
    ...
    fn charge_read_cost(&self, key: &CacheKey, value: Option<&CacheValue>) -> CostChange {
        let cost_change = match key {
            ...
            _=> 
                CostChange::deduct(gas::get_cost(key.len(), value.as_ref().map_or(0, |v| v.len())))
        };
        *self.read_gas.borrow_mut() += cost_change;
        cost_change
    }
...
impl CacheKey {
    ...
    pub fn len(&self) -> usize {
        match self {
            ...
            CacheKey::Balance(_) |
            CacheKey::ContractCode(_) |
            CacheKey::CBIVersion(_) => gas::ACCOUNT_STATE_KEY_LENGTH,
        }
    }
